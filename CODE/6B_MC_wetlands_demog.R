...
calc_dist <- function(data, wetlands.sp){
...
spade_nnd <- calc_dist(spts, wetlands.sp) #calc dist of each frog to nearest pond 
...
for (i in c(1:1000)) {
  ...
  rand <- SpatialPoints(rand)
  rand_nnd<- calc_dist(rand, wetlands.sp)
...
fx1 <- c(47, 249, 290, 309, 425, 692)
fx2 <- c(219, 274, 301, 335, 430, 900)
fy1 <- c(rep(0, 6))
fy2 <- c(rep(1, 6))

fshade = data.frame(fx1, fx2, fy1, fy2)
setwd("~/Dropbox/Publications_Work/Ecosphere_REV/DATA")
write.csv(fshade, file="significant_xvals_NBA_wetlands.csv", row.names=FALSE)
...
png("Wetland_NBA.png", units="in", width=6, height=5, res=600)
pS
dev.off()
